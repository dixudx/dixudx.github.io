...
function search(query){return idx.search(query);};$(document).ready(function(){$('.new-comment').submit(function(){var form=this;$(form).addClass('loading');$('input[type="submit"]:enabled').addClass('hidden');$('input[type="submit"]:disabled').removeClass('hidden');var api='staticman-g6v1x01.herokuapp.com';var gitProvider='github';var username='dixudx';var repo='dixudx-blog-comments';var branch='master';$.ajax({type:$(this).attr('method'),url:['https:/',api,'v3/entry',gitProvider,username,repo,branch,'comments'].join('/'),data:$(this).serialize(),contentType:'application/x-www-form-urlencoded',success:function(data){showAlert('success');setTimeout(function(){clearForm()},3000);$(form).removeClass('loading');},error:function(err){console.log(err);showAlert('failed');$(form).removeClass('loading');}});return false;});function showAlert(msg){if(msg=='success'){$('.submit-notice').addClass('submit-success')
